...
* Strict Turnstile Model：$\Delta_j$ 可正可负，需满足但是任意时刻 $\vec X\in\R_{\geq 0}^n$（也就是 $\Delta_j\geq -x_{i_j}$）
...
$$
当 $d = \Theta\qty(\ln n)$ 的时候，也就是 $\delta = n^{\Theta(1)}$ 的时候，算法的正确概率就能相当高。所以 Count Min Sketch 只需要 $\order{k\log n}$ 个计数器就可以完成 $(k, l_1)$ 单点查询
...
这表明当 $d = \Theta(\ln n)$, $w = \Theta(k^2)$ 时就可以以很高的概率解决 $2$ 单点查询问题，需要 $\order{k^2\ln n}$ 个计数器。假设每个计数器的值都不超过 $m$，那么它的空间复杂度就是 $\order{k^2\ln n\ln m}$
