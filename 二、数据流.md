...
* 图流模型：$\Delta_j\in\pm 1$，且任意时刻 $\vec X\in\N^n$。这时 $\set{1, 2, \cdots, n}$ 对应图中潜在边的集合，也就表明动态图问题中可能没有某条边或者关于某条边有很多重边，但是不可能某条边有负数条

### 推广的众数查询

$(k, l_1)$ 点查询问题指：任意时刻询问任意的 $i\in\set{1, 2, \cdots, n}$，需要输出 $\tilde x_i = x_i + \frac 1k\norm{X}_1$
...
**引理：**设 $\mathcal A$ 是一个 $(3k, l_1)$ 众数查询问题的算法，其失败概率不超过 $\frac \delta n$，且消耗空间为 $s$ 比特。那么可以构造出一种解决 $(k, l_1)$ 众数查询问题的算法 $\mathcal A'$，失败概率不超过 $\delta$，消耗 $s + \order{k\log n}$ 比特

**证明：**我们这样构造 $\mathcal A'$：遍历 $i = 1, 2, \cdots, n$
