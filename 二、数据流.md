...
**定理：**以至少 $\frac 1n$ 的概率本算法输出的 $\vec z$ 满足 $\norm{\vec X - \vec z}_z\leq(1 + 5\epsilon){\rm err}_2^k(\vec X)$

> 如果需要相对误差不超过 $\epsilon$，将算法描述中的 $w$ 改成 $\frac{3k}{(\epsilon / 5}^2) = \frac{75k}{\epsilon^2}$ 即可

为了证明这个定理，我们需要先证明两个引理：

**引理 1：**以至少 $1 - \frac 1n$ 的概率，Count Sketch 算法给出的 $\tilde X$ 能保证对于每一个 $i = 1, 2, \cdots, n$ 都有 $\abs{\tilde x_i - x_i}\leq\frac\epsilon{\sqrt k}{\rm err}_2^k(x)$
