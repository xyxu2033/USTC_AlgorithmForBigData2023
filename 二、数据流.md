...
* 如果 $q = p^r$ 是质数的幂（也就是有且仅有一个质因数），且 $a = b = q$，在 $q$ 阶有限域中，全体度数不超过 $k - 1$ 的多项式组成的函数族 $\mathcal H_{\rm poly} = \set{h | h(x) = d_0 + d_1x + \cdots + d_{k - 1}x^{k - 1}}$ 组成一个独立哈希函数族，而存储这个哈希函数族内的元素仅需要存储这 $k$ 个系数即可，只需要 $k\log q$ 个比特
...
>
> 度数不超过 $r - 1$ 的全体模 $p$ 意义下多项式的集合 $\set{A | A(x) = a_0 + a_1x + \cdots + a_{r - 1}x^{r - 1}, \vec a\in\mathbb F_p^r}$ 在以下加法与乘法意义下是一个阶为 $p^r$ 的域 $\mathbb F_{p^r}$：
>
> * 加法 $+$ 是普通多项式的加法：
> $$
...
2. 当数据流中流入一个数 $x$ 时
   * 如果 $x = I_j$，那么 $c_j\gets c_j + 1$
   * 如果 $x$ 不在 $\vec I$ 中但 $c_j = 0$，那么 $I_j\gets x$ 且 $c_j\gets 1$
   * 如果 $x$ 不在 $\vec I$ 中且 $\vec c$ 中所有的元素都非零，那么让 $\vec c$ 中所有的数都减少 $1$
3. 查询时，输出 $\vec I$