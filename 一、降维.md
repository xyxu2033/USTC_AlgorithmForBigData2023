...
$$
假设 $\sigma_i > \sigma_2$ 而且 $k$ 充分大，从直觉上有 $B^k\approx \sigma_1^{2k}\vec v_1\vec v_1^\intercal$，而 $\sigma_1^{2k}\vec v_1\vec v_1^\intercal$ 的第一列是 $\vec v_1$ 的 $\sigma_1^{2k}v_{1,1}$ 倍，这里 $v_{1,1}$ 是 $\vec v_1$ 的第一个分量。再考虑到 $\vec v_1$ 是一个单位向量，我们可以得到一个简易的算法：
...
回忆前文中提到 $B^k = \sum_{i = 1}^d\sigma_i^{2k}\vec v_i\vec v_i^\intercal$，所以 $B^k\vec x = \sum_{i = 1}^d\sigma_i^{2k}\alpha_i\vec v_i$。由勾股定理：
$$
\norm{B^k\vec x}_2^2 = \norm{\sum_{i = 1}^d\sigma_i^{2k}\alpha_i\vec v_i}_2^2 = \sum_{i = 1}^d\qty(\sigma_i^{2k}\alpha_i)^2\geq\sigma_1^{4k}\alpha_1
$$
...
计算前 $k$ 个奇异向量的方法通常是启发式的，很难得到一个有很强理论保证的算法（正如一般 5 次以上多项式方程没有初等公式解一样）。这里不加分析地给出一个启发式算法：

1. 随机选选取一个向量 $\vec r$，同时计算 $B = A^\intercal A$
