...
>
> 原命题的 $c$ 比较令人迷惑。在我的表述中，$c = \frac 15$，也就是说投影到 $\left\lceil\frac{15\ln n}{\epsilon^2}\right\rceil$ 维时两点之间的距离从 *理论* 上几乎一定保持得很好。一些更强的结论要求 $c = \frac 1{96}$ 才能在 *理论* 上达到，如此巨大的常数使得对于实际应用中的 $d$（比如 MNIST 中的 $d = 784$），随机投影的目标维数 $k$ 可能比 $d$ 还要大才能给出数据集随机投影的 *理论* 保证
>
> 但在工程实践中不一定要这么死板，命题只是关于概率下界的，用了大量的放缩，即使维数少个几十倍，从 *实践* 上依然能有很好的结果。本命题的意义在于，对于任意的原数据维数 $d$，只要随机投影到大概 $\order{\left\lceil\frac{3\ln n}{c\epsilon^2}\right\rceil} = \order{\frac {\log n}{\epsilon^2}}$ 维，任意两点之间的距离就可以保持了。也就是说可以逐一增加随机投影的维数，直到距离保持特性比较好时停止，停止时投影的维数只有 $\order{\frac{\log n}{\epsilon^2}}$
...
**定理（高斯环）：**设随机向量 $\vec X$ 服从 $d$ 维标准高斯球分布，对于任意 $\beta\leq\frac{\sqrt d}5$，
$$
\mathbb P\set{\sqrt d - \beta\leq\norm{\vec X}\leq \sqrt d + \beta}\geq 1 - 2e^{-c\beta^2}
$$
...
我们对高斯环定理的一些表述做一些转化
$$
\sqrt d - \beta\leq \norm{\vec X}_2\leq \sqrt d + \beta\iff \abs{r - \sqrt d}\leq \beta\\
\mathbb P\qty[\sqrt d - \beta\leq\norm{\vec X}_2\leq \sqrt d + \beta]\geq 1 - 2e^{-c\beta^2}\\
\iff \mathbb P\qty[\abs{\norm{\vec X}_2 - \sqrt d} > \beta] \leq 2e^{-c\beta^2}\\
...
\end{aligned}
$$
对于任意的 $\lambda\in\qty(0, \frac 1{10})$ 上式都需要成立，为了给出强的约束我们取 $\lambda$ 使得上式右端尽可能小。联系到 $\beta\leq \frac{\sqrt d}5$ 知 $\frac{\beta}{2\sqrt d}\in\qty(0, 10)$，表明我们可以将 $\lambda = \frac{\beta}{2\sqrt d}$ 代入上式，代入后得到
$$
...
最近邻搜索问题需要维护一个数据库，数据库内包含 $n$ 个数据点 $\vec a_1, \vec a_2, \cdots, \vec a_n\in\R^d$。数据库同时还需要处理询问。每个询问输入一个 $\vec x\in\R^d$，数据库需要输出与 $\vec x$ 距离最近的点
$$
